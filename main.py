...
def create_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(new_user)
    return {"message": f"Usuário {new_user.username} criado com sucesso!"}
...
def create_pedido(pedido: schemas.PedidoCreate, db: Session = Depends(database.get_db)):
    ...
    usuario = db.query(models.User).filter(models.User.id == pedido.usuario_id).first()
    if not usuario:
...
"""
@app.patch("/pedidos/{pedido_id}/forma_pagamento", response_model=schemas.Pedido)
def update_forma_pagamento_pedido(
    pedido_id: int, 
    nova_forma: schemas.PedidoCompleto,
    db: Session = Depends(database.get_db)
):
    pedido = db.query(models.Pedido).filter(models.Pedido.id == pedido_id).first()
    ...
    pedido.forma_pagamento = nova_forma
